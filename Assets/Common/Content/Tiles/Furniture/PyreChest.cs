...
namespace PyreModPlus.Assets.Common.Content.Tiles.Furniture
{
	public class PyreChest : ModTile
	{
		public override void SetStaticDefaults() {
			...
			AddMapEntry(new Color(255, 165, 0), this.GetLocalization("MapEntry0"), MapChestName);
			AddMapEntry(new Color(255, 165, 0), this.GetLocalization("MapEntry1"), MapChestName);
			...
			RegisterItemDrop(ModContent.ItemType<Items.Placeable.Furniture.PyreChest>(), 1);
		...
		public override bool RightClick(int i, int j) {
			...
			if (Main.netMode == NetmodeID.MultiplayerClient && !isLocked) {
				...
				if (isLocked) {
					...
					int key = ModContent.ItemType<Items.Other.PyreKey>();
		...
		public override void MouseOver(int i, int j) {
			...
			if (chest < 0) {
				...
				if (player.cursorItemIconText == defaultName) {
					player.cursorItemIconID = ModContent.ItemType<Items.Placeable.Furniture.PyreChest>();
					if (Main.tile[left, top].TileFrameX / 36 == 1) {
						player.cursorItemIconID = ModContent.ItemType<Items.Other.PyreKey>();
					}
