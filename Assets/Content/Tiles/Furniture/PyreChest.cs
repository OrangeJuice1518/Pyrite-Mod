...
namespace PyreModPlus.Assets.Content.Tiles.Furniture
{
		...
		public override bool RightClick(int i, int j) {
			...
			if (Main.netMode == NetmodeID.MultiplayerClient && !isLocked) {
				...
				if (isLocked) {
					...
					int key = ModContent.ItemType<Content.Items.PyreKey>();
			...
			if (chest < 0) {
				...
				if (player.cursorItemIconText == defaultName) {
					...
					if (Main.tile[left, top].TileFrameX / 36 == 1) {
						player.cursorItemIconID = ModContent.ItemType<Content.Items.PyreKey>();
					}
